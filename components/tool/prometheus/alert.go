...
** General Purpose **
It lists alerts from the Alertmanager associated with a Prometheus instance.
...
func (t *AlertTool) Invoke(ctx context.Context, params *AlertParams) (result string, err error) {
	...
	if params.Fingerprint != "" {
		filtered := make([]gettableAlert, 0, 1)
		for _, a := range alerts {
			if a.Fingerprint == params.Fingerprint {
				filtered = append(filtered, a)
			}
		}
		alerts = filtered
	} else if params.State != "" {
		filtered := make([]gettableAlert, 0, len(alerts))
		for _, a := range alerts {
			if a.Status.State == params.State {
				filtered = append(filtered, a)
			}
		}
		alerts = filtered
...
func NewAlertTool(ctx context.Context, configs Configs) (*AlertTool, error) {
	...
	listTool := &AlertTool{alertmanagerBaseTool: base}
	t, err := utils.InferTool("prometheus_alert", fmt.Sprintf("%s\n%s", alertDescription, listOutputGuidance), listTool.Invoke)
	if err != nil {
		return nil, err
	}
