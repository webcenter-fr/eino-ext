...
func TestCheckEmptyConfigs(t *testing.T) {
	...
	if results[0].Component != "argocd" {
		t.Errorf("expected component argocd, got %s", results[0].Component)
	}
}
...
func TestCheckInvalidInstance(t *testing.T) {
	...
	if len(results) != 10 {
		t.Errorf("expected 10 results (one per tool), got %d", len(results))
	}
}
...
func TestCheckClientErrorResults(t *testing.T) {
	...
	if len(r) != 10 {
		t.Fatalf("expected 10 results, got %d", len(r))
	}
	for i, rr := range r {
