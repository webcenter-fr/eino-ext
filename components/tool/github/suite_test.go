...
func (s *GitHubToolTestSuite) SetupSuite() {
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(path, "/comments") && strings.Contains(path, "/issues/1/"):
			...
			w.Write([]byte(`{"id": 100, "html_url": "https://github.com/testowner/testrepo/issues/1#issuecomment-100"}`))
		...
		case strings.HasSuffix(path, "/issues") && strings.Contains(path, "/repos/"):
			if r.Method == http.MethodPost {
				w.Header().Set("Content-Type", "application/json")
				w.WriteHeader(http.StatusCreated)
				w.Write([]byte(`{"number": 3, "title": "New issue", "state": "open", "html_url": "https://github.com/testowner/testrepo/issues/3"}`))
				return
			}
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
		...
		case strings.HasSuffix(path, "/pulls/10/reviews"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"id": 300, "state": "APPROVED", "html_url": "https://github.com/testowner/testrepo/pull/10#pullrequestreview-300"}`))
		case strings.Contains(path, "/pulls/10/comments"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusCreated)
			w.Write([]byte(`{"id": 400, "path": "main.go", "line": 42, "html_url": "https://github.com/testowner/testrepo/pull/10#discussion_r400"}`))
		case strings.HasSuffix(path, "/pulls/10/requested_reviewers"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusCreated)
			w.Write([]byte(`{"number": 10, "requested_reviewers": [{"login": "reviewer1"}, {"login": "reviewer2"}]}`))
		...
		case strings.Contains(path, "/issues/10/comments"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusCreated)
			w.Write([]byte(`{"id": 200, "html_url": "https://github.com/testowner/testrepo/pull/10#issuecomment-200"}`))
		case strings.HasSuffix(path, "/pulls") && strings.Contains(path, "/repos/"):
			if r.Method == http.MethodPost {
				w.Header().Set("Content-Type", "application/json")
				w.WriteHeader(http.StatusCreated)
				w.Write([]byte(`{"number": 11, "title": "New PR", "state": "open", "html_url": "https://github.com/testowner/testrepo/pull/11", "draft": false}`))
				return
			}
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
		...
		case strings.HasSuffix(path, "/repos/testowner/testrepo/hooks/500"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"id": 500, "events": ["push", "pull_request"], "active": true}`))
		case strings.HasSuffix(path, "/hooks") && strings.Contains(path, "/repos/"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusCreated)
			w.Write([]byte(`{"id": 500, "events": ["push"], "active": true}`))
		case strings.HasSuffix(path, "/releases") && strings.Contains(path, "/repos/"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusCreated)
			w.Write([]byte(`{"id": 600, "tag_name": "v1.0.0", "name": "v1.0.0 Release", "html_url": "https://github.com/testowner/testrepo/releases/tag/v1.0.0", "draft": false, "prerelease": false}`))
		...
		case strings.HasSuffix(path, "/git/refs") && strings.Contains(path, "/repos/"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusCreated)
			w.Write([]byte(`{"ref": "refs/heads/new-branch", "object": {"sha": "abc123def456"}}`))
		case strings.HasSuffix(path, "/repos/testowner/testrepo"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"name": "testrepo", "full_name": "testowner/testrepo", "private": false, "default_branch": "main", "html_url": "https://github.com/testowner/testrepo"}`))
		case strings.HasSuffix(path, "/repos/testowner/testrepo-remote"):
			w.Header().Set("Content-Type", "application/json")
			w.WriteHeader(http.StatusOK)
			w.Write([]byte(`{"name": "testrepo-remote", "full_name": "testowner/testrepo-remote", "private": false, "default_branch": "main"}`))
		default:
			w.Header().Set("Content-Type", "application/json")
